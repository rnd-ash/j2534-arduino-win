...
int iso15765_handler::send_payload(PASSTHRU_MSG* pMsg, unsigned long* pNumMsgs, unsigned long Timeout, unsigned long baud)
{
	...
	// Set the arduinos baud rate - Always send this. The arduino will ignore the request if baud is already OK, so we don't keep resetting the MCP chip!
	...
	for (unsigned long i = 0; i < *pNumMsgs; i++) {
		...
		while (send_queue.size() > 0) {/* Waiting - block more writes */}
	}
...
void iso15765_handler::processPayload(DATA_PAYLOAD* p)
{
	...
	if (matched) {
		...
		case 0x00:
			last_ecu_resp_time = GetTickCount();
			...
			recv_queue.push(m); // Add ECU response to queue
			break;
		case 0x10:
			last_ecu_resp_time = GetTickCount();
			num_bytes = p->args[5];
		...
		case 0x30:
			if (num_bytes_to_send > 0) {
				...
				if (num_bytes_sent >= num_bytes_to_send) {
					...
					memcpy(&m.Data[0], &p->args[0], 4); // Copy CID;
					recv_queue.push(m);
					send_queue.pop(); // Finished sending - pop the queue
					sending = false; // Unlock the send lock
				}
			}
			last_ecu_resp_time = GetTickCount();
			break;
...
int iso15765_handler::internal_send_payload()
{
	...
	if (sending && GetTickCount() - last_ecu_resp_time > 100) { // ECU took too long to respond. Missing packet? Try and resend the data
		sending = false;
		LOGGER.logWarn("15765_SEND", "ECU response not found, resending packet!");
	}

	// Only send a payload if we can, if not, then keep the stack
	if (send_queue.size() > 0 && !sending) {
		LOGGER.logDebug("SEND_QUEUE", "Send Queue size: %lu", send_queue.size());
		...
		if (m->DataSize <= 11) {
			...
			send_queue.pop(); // Sent a payload, pop the queue
			last_ecu_resp_time = GetTickCount();
			sending = false;
			return STATUS_NOERROR;
		}
		else {
			sending = true; // Multi frame payload, lock the send lock!
...
int msg_handler::read_messages(PASSTHRU_MSG* pMsg, unsigned long* pNumMsgs, unsigned long Timeout)
{
	...
	LOGGER.logDebug("RECV_QUEUE", "Recv Queue size: %lu", recv_queue.size());
	// Copy at most how many messages the application wants back to it, whilst popping the queue
	...
	return STATUS_NOERROR; // Copied some messages, all good!
}
